...
class ScheduledPostsModule(BotModule):
        ...
        for group in self.target_groups():
            if as_bool(group.get("daily_enabled"), True):
                run_at = self.random_time(
                    group.get("daily_window_start") or self.sheets.value("daily_window_start", "20:00"),
                    group.get("daily_window_end") or self.sheets.value("daily_window_end", "23:59"),
                )
            ...
            if as_bool(group.get("video_enabled"), False):
                run_at = self.random_time(
                    group.get("video_window_start") or self.sheets.value("video_window_start", "20:00"),
                    group.get("video_window_end") or self.sheets.value("video_window_end", "23:59"),
                )
    ...
    def send_boot_messages(self):
        if not as_bool(self.sheets.value("send_on_boot", "false"), False):
            return
        for group in self.target_groups():
    ...
    def send_random_message(self, group, force=False):
        ...
        send_if_silent = as_bool(group.get("send_if_silent") or self.sheets.value("send_if_silent", "false"), False)
        ...
        candidates = [
            row for row in self.sheets.enabled_rows("messages")
    ...
    def copy_random_video(self, group):
        ...
        candidates = [
            row for row in self.sheets.enabled_rows("video_messages")
